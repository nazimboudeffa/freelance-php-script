...
class Search extends Controller {
	...
	function Search()
	{
	   ...
	   //$this->output->enable_profiler=true;		
		...
		$this->outputData['popular_provid'] = $this->skills_model->getPopularSearch('user');
...
// --------------------------------------------------------------------
	...
	{	
	    ...
	    $group1 = $this->input->get_post('group1');
	    $this->session->set_userdata('group1',$group1);
	...
	if($group1=='Projects')
	{
		...
		$config['base_url'] 	= $this->config->item('base_url')."?group1=".$group1."&category=".$category."&c=search&keyword=".$keyword;
	...
	//Load Language
		...
		$page_rows =$this->session->userdata('show_num');
		$max = array($page_rows,($page - 1) * $page_rows); 
        $condition=array('categories.category_name'=>$this->input->get('category'));
	    $category=$this->search_model->getCategory($condition);
		...
		if($this->input->get('category'))
		   $like1 =array('user_categories.user_categories'=>$cate_id);
		else
		   $like1 = '';   
		...
		/*if(isset($keyword))
		{
			...
			$this->load->library('pagination');
			$config['base_url'] 	= $this->config->item('base_url')."?c=search&keyword=".$keyword.'&m=professional';
		...
		if(isset($category))
		{
			$this->outputData['users'] = $users;
			$this->load->library('pagination');
			$config['base_url'] 	= $this->config->item('base_url')."?c=search&keyword=&category=".$category.'&m=professional';
			$config['total_rows'] 	= $users1->num_rows();		
			$config['per_page'] = $page_rows; 
			$config['cur_page'] = $page;
			$this->pagination->initialize($config);	
		...
		if(isset($cate_id)){
		$config['base_url'] 	= $this->config->item('base_url')."?group1=".$group1."c=search&keyword=".$keyword."&category=".$cate_id.'&m=professional';
		}
		else
		{
		$config['base_url'] 	= $this->config->item('base_url')."?group1=".$group1."c=search&keyword=".$keyword."&category=".$category.'&m=professional';
		}
		$this->outputData['users'] = $users;
		$this->load->library('pagination');
		if(!isset($keyword))
