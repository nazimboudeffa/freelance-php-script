		...
		?>
			<tr>	
			  ...
			  <?php foreach($usersList->result() as $users) { if($users->id == $transactions->creator_id) {?>
			      <a href="<?php echo site_url().$transactions->creator_id; ?>"><td><?php echo $users->user_name; ?></td></a>
			      <?php } } ?>	  
			  <td> <?php
			  ...
			  <td><?php echo get_date($transactions->request_date); ?></td>
			  <td><?php echo $transactions->status; ?></td>
        	
			 <td>
			    <a href="<?php echo admin_url('payments/acceptEscrow/'.$transactions->transaction_id);?>"><?php echo $this->lang->line('Accept'); ?></a>&nbsp;&nbsp;&nbsp;
			    <a href="<?php echo admin_url('payments/deniedEscrow/'.$transactions->transaction_id);?>"><?php echo $this->lang->line('Denied'); ?></a>
			  </td>			  			  
        	</tr>
        <?php
				}//Foreach End
			...
			{ 			
			  echo '<tr><td colspan="5">'.$this->lang->line('No Transaction Found').'</td></tr>'; 
			}
		?>
		</table>
      </div>
