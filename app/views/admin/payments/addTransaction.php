    ...
    <div class="clsMainSettings">
	 ...
	 <h3><?php echo $this->lang->line('Add Transaction'); ?></h3>
	  ...
	  <form method="post" action="<?php echo admin_url('payments/addTransaction')?>">
		...
		<b id="uname">  <select name="from_username" class="usertype" id="" onchange="javascript:loadFromuser()">
        ...
        <tr>
         ...
         <b id="uname1">  <select name="to_username" class="usertype" id="" onchange="javascript:loadFromuser()">
...
function loadTouser()
{
   ...
   new Ajax.Request('<?php echo base_url()."index.php/".$this->config->item('admin_controllers_folder').'/payments/load_users1/';?>'+utype,
  {
    ...
    onSuccess: function(transport){
      var response = transport.responseText || "no response text";
	  document.getElementById('uname1').innerHTML = response
    },
    onFailure: function(){ alert('Something went wrong...') }
  });
